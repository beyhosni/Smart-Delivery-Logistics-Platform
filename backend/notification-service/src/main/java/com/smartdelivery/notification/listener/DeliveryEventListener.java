...
public class DeliveryEventListener {
    ...
    public void handleDeliveryEvent(NotificationEvent event) {
        log.info("Received delivery event: {} for user: {}", event.getType(), event.getUserId());

        try {
            // Créer les notifications pour chaque canal activé
            createNotificationsForEvent(event);
        } catch (Exception e) {
            log.error("Error processing delivery event: {}", event.getEventId(), e);
        }
    }
